...
async def register_user(request: UserRegistrationRequest, db: Session = Depends(get_db)):
    ...
    # --- CAMBIO SUGERIDO ---
    # Después de registrar, inicia sesión automáticamente al usuario.
    # Crea y devuelve un token de acceso junto con los datos del usuario.
...
async def login_user(request: LoginRequest, db: Session = Depends(get_db)):
    ...
    # Esta implementación ya es correcta. La mantenemos como referencia.
    return {
        "access_token": access_token, 
        "token_type": "bearer",
        "user": {
...
async def social_login(request: SocialLoginRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        ...
        if existing_user_by_email:
            ...
            # Notificar a los otros servicios que se ha creado un nuevo usuario.
            ...
            try:
                async with httpx.AsyncClient() as client:
                    user_profile_data = {
                        "id_usuario": user.id_usuario,
                        "nombre": user.nombre,
                        "apellido": user.apellido or '', # Aseguramos que no sea None
                        "correo": user.correo,
                    }
                    response = await client.post(f"{USER_SERVICE_URL}/", json=user_profile_data)
                    response.raise_for_status()
            except httpx.RequestError as e:
                # En un sistema real, se debería manejar este fallo (ej. reintentos, compensación)
                print(f"Error al notificar al user-service durante el social-login: {e}")
                raise HTTPException(status_code=500, detail="Error al crear el perfil de usuario en servicios internos.")

    # 3. Crear y devolver el token de acceso de nuestra aplicación
    # --- CAMBIO SUGERIDO ---
    # Devuelve el token Y los datos del usuario para ser consistente.
    access_token = create_access_token(data={"sub": user.correo, "id": user.id_usuario}, role=user.rol)
    return {
        "access_token": access_token,
        "token_type": "bearer",
